...
| `TV-MTR-07` | REQ-PUB-02 | Ingest HTML | Ingest document possessing `@media(dark)`. Observe Fidelity threshold. | Fidelity calculator tracks Dark mode inversion. Logs warning but permits publish under 'allow-below-threshold' logic. | Fidelity threshold JSON output log. |
...
| `TV-MTR-10` | REQ-WID-01 | Upload screen | Upload raw JS widget locally. Upload repo-path component. | Raw JS fails validation sandbox. Repo path authenticates and embeds successfully. | Rejection log / Success DB commit. |

...
| `TSK-003` | REQ-PUB-[01,02] | Integrate Fidelity Diff engine (Dark/Light tracking) and lock Publish screen to Active production theme. | Claude (UI) | Configurable Studio Settings object fetching threshold modes (allow/disallow). | Publish screen ignores Preview swatches safely. Fidelity captures dark/light delta correctly natively. |
...
| `TSK-006` | REQ-WID-01 | Sandbox widget uploads to reject raw manual JS. Enable repo-first widget discovery and standalone/embedded mapping. | Codex (Integr) | `tests/integration/widgets` repository setup. | Unit tests reject `<script>` tags natively but allow local path logic extraction. |

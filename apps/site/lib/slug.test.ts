...
import { resolveCmsRoute, resolveCmsSlug } from "./slug";
...
describe("resolveCmsSlug", () => {
  ...
  it("returns locale-aware route metadata", () => {
    expect(resolveCmsRoute(["en", "home"])).toEqual({
      locale: "en",
      slug: "home"
    });
    expect(resolveCmsRoute(["products", "cpq"])).toEqual({
      locale: "en",
      slug: "products/cpq"
    });
  });
